(() => {
  ...
  function directPersonDebtMetrics(id,currency){ const ts=directTransfers(currency); return {owes:ts.filter(t=>t.from===id).reduce((a,t)=>a+t.amount,0), owedTo:ts.filter(t=>t.to===id).reduce((a,t)=>a+t.amount,0)}; }
  ...
  function renderDashboard(){
    ...
    const peopleCards=ps.map(p=>{const m=personMetrics(p.id);return `<div class="card person-card" data-person="${p.id}"><div class="row-between"><div class="top-payer"><div class="avatar">${esc(p.name).slice(0,1)}</div><strong>${esc(p.name)}</strong></div><span class="muted">פרטים ←</span></div><div class="person-metrics"><div class="mini-metric"><small>שילם</small>${Object.keys(CURRENCIES).map(c=>`<div>${money(m[c].paid,c)}</div>`).join('')}</div><div class="mini-metric"><small>חייב לאחרים</small>${Object.keys(CURRENCIES).map(c=>{const d=directPersonDebtMetrics(p.id,c);return `<div class="kpi-negative">${money(d.owes,c)}</div>`}).join('')}</div><div class="mini-metric"><small>אחרים חייבים לו</small>${Object.keys(CURRENCIES).map(c=>{const d=directPersonDebtMetrics(p.id,c);return `<div class="kpi-positive">${money(d.owedTo,c)}</div>`}).join('')}</div></div></div>`}).join('');
